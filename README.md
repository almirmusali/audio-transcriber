...
- Имя TXT-файла подбирается по смыслу записи (Claude Sonnet по началу расшифровки)
- Кнопка «💡 Идея» — расшифровка уходит в «Банк идей» (:8771), он разбирает её на идеи
- Всё работает офлайн; аудио никуда не уходит (кроме этих двух кнопок — они ходят к модели)

## Установка готового приложения

Скачай `Транскрибер-1.1.0-arm64.dmg` (версия **1.1.0**, см. [CHANGELOG.md](CHANGELOG.md)), открой, перетащи «Транскрибер» в «Программы».
